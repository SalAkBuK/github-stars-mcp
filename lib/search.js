...
export function tokenize(text) {
  ...
  // Match words, alphanumeric tokens, and hyphenated/underscore terms
  const wordMatches = raw.match(/[a-z0-9]+(?:[-_][a-z0-9]+)*/g) || [];
...
export class BM25SearchEngine {
  ...
  search(query, options = {}) {
    ...
    const minScore = typeof options.min_score === "number" ? options.min_score : 0.1;
    ...
    for (let i = 0; i < this.documents.length; i++) {
      ...
      if (score >= minScore) {
        results.push({
          ...
          url: r.url || r.html_url || `https://github.com/${r.name || r.full_name}`,
...
export async function loadCatalogRepos(filePath) {
  const resolvedPath = filePath || path.join(process.cwd(), "GITHUB_STARS.md");
  try {
...
function parseSimpleCatalog(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentRepo) {
      ...
      if (trimmed && !currentRepo.summary && !trimmed.startsWith("#") && !trimmed.startsWith("<")) {
