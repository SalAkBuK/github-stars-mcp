...
export function classifyFreshness(repo, referenceDate = new Date()) {
  ...
  if (Boolean(repo.archived)) {
    // Repo is explicitly archived
    const dateStr = repo.pushed_at || repo.last_commit_at || repo.updated_at;
    let days = null;
    if (dateStr) {
    ...
    } else if (typeof repo.days_since_push === "number") {
      days = repo.days_since_push;
    }
    return { freshness: "ARCHIVED", days };
  }

  // Check explicit days_since_push or calculate from timestamp
  let days = null;
  if (typeof repo.days_since_push === "number") {
    days = Math.max(0, Math.floor(repo.days_since_push));
  } else {
    const dateStr =
      repo.pushed_at ||
      repo.last_commit_at ||
...
export function auditRepository(repo, referenceDate = new Date()) {
  ...
  const rawLicense = typeof repo.license === "object" && repo.license !== null
    ? (repo.license.spdx_id || repo.license.name || repo.license.key || "Unknown")
...
export function auditRepositories(repos = [], options = {}) {
  ...
  const minHealth = typeof options.min_health_score === "number"
    ? options.min_health_score
    : null;
