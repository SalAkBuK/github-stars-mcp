...
/**
 * High-precision regex patterns for detecting languages in natural language pitches
 * Prevents false positives on common English words ('to go', 'cargo containers', 'hidden gem', 'clean')
 */
const PITCH_PATTERNS = {
  go: [
    /\bgolang\b/i,
    /\b(?:written\s+in|built\s+with|built\s+in|in)\s+go\b/i,
    /\bgo\s+(?:library|package|client|server|daemon|cli|sdk|module|backend|tool)\b/i,
    /[\(\[]go[\)\]]/i,
  ],
  golang: [
    /\bgolang\b/i,
  ...
  rust: [/\brust\b/i, /\bcargo\s+(?:add|build|install)\b/i],
  rs: [/\brust\b/i, /\bcargo\s+(?:add|build|install)\b/i],
  python: [/\bpython\b/i, /\b(?:pip|uv)\s+install\b/i],
  py: [/\bpython\b/i, /\b(?:pip|uv)\s+install\b/i],
  ruby: [/\bruby\b/i, /\bgem\s+install\b/i],
  rb: [/\bruby\b/i, /\bgem\s+install\b/i],
  c: [/\b(?:written\s+in|built\s+with|built\s+in|in)\s+c\b/i, /[\(\[]c[\)\]]/i],
  "c++": [/\bc\+\+\b/i, /\bcpp\b/i],
  cpp: [/\bc\+\+\b/i, /\bcpp\b/i],
  "c#": [/\bc#\b/i, /\bcsharp\b/i, /\bdotnet\b/i, /\b\.net\b/i],
  csharp: [/\bc#\b/i, /\bcsharp\b/i, /\bdotnet\b/i, /\b\.net\b/i],
  cs: [/\bc#\b/i, /\bcsharp\b/i, /\bdotnet\b/i, /\b\.net\b/i],
  typescript: [/\btypescript\b/i],
  ts: [/\btypescript\b/i],
  javascript: [/\bjavascript\b/i, /\bnode\.?js\b/i],
  js: [/\bjavascript\b/i, /\bnode\.?js\b/i],
  java: [/\bjava\b/i, /\bjvm\b/i],
  kotlin: [/\bkotlin\b/i],
  kt: [/\bkotlin\b/i],
  swift: [/\bswift\b/i],
  php: [/\bphp\b/i],
  shell: [/\bbash\b/i, /\bzsh\b/i, /\bshell\s+script/i],
  bash: [/\bbash\b/i, /\bzsh\b/i, /\bshell\s+script/i],
  sh: [/\bbash\b/i, /\bzsh\b/i, /\bshell\s+script/i],
};
...
export function inferInstallCommand(repo) {
  ...
  const fullName =
    repo.full_name ||
    (repo.name && repo.name.includes("/") ? repo.name : null) ||
    (repo.url ? repo.url.replace(/^https?:\/\/github\.com\//i, "").replace(/\.git$/i, "") : name);
  ...
  if (lang === "go" || tags.includes("golang") || tags.includes("go")) {
    return `go install github.com/${fullName}@latest`;
  }
  ...
  const url = repo.url || repo.html_url || (fullName ? `https://github.com/${fullName}` : `https://github.com/${baseName}`);
  const cleanUrl = url.endsWith(".git") ? url : `${url}.git`;
  return `git clone ${cleanUrl}`;
}
...
function matchesLanguage(repo, requestedLang) {
  ...
  const patterns = PITCH_PATTERNS[target] || [];
...
export async function recommendStack(options = {}) {
  ...
  let searchResults = engine.search(task_description, {
    limit: limit * 2,
    min_score: 0.1,
  });

  if (searchResults.length === 0) {
    searchResults = engine.search(task_description, {
      limit: limit * 2,
      min_score: 0.01,
    });
  }
  ...
  for (let i = 0; i < searchResults.length; i++) {
    ...
    const tagsSlice = (Array.isArray(hit.tags) ? hit.tags : []).slice(0, 4);
